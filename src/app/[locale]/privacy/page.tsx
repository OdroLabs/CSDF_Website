...
export async function generateMetadata({
  ...
  const title = s(settings, "privacy_title", locale) || dict.nav.privacy;
  return {
...
export default async function PrivacyPolicyPage({ params }: { params: { locale: Locale } }) {
  ...
  const title = s(settings, "privacy_title", locale) || dict.nav.privacy;
  const content = s(settings, "privacy_content", locale);

  return (
    <>
      <PageHero
        title={title}
        breadcrumb={{ homeHref: `/${locale}`, homeLabel: dict.nav.home, currentLabel: dict.nav.privacy }}
      />
