import type { Locale } from "./i18n";

/**
 * Interface labels — buttons, menu items, form fields and other short strings.
 *
 * These are the built-in defaults. Every one of them can be overridden per
 * language from the admin panel (Site Settings → Labels & Translations); the
 * values here are used whenever an override is blank. Longer page content
 * (headings, body text, images) lives in Site Settings instead — see
 * `src/lib/settings.ts`.
 */
const en = {
  nav: {
    home: "Home",
    about: "About Us",
    projects: "Projects",
    services: "Our Services",
    servicesOverview: "All Services",
    publications: "Publications",
    news: "News",
    events: "Events & Gallery",
    contact: "Contact Us",
    donate: "Donation",
    business: "Community Business",
    suggestions: "Suggestions",
    media: "News & Media",
    getInvolved: "Get Involved",
    menu: "Menu",
    privacy: "Privacy Policy",
    terms: "Terms & Conditions",
  },
  common: {
    readMore: "Read More",
    learnMore: "Learn More",
    viewAll: "View All",
    submit: "Submit",
    send: "Send",
    name: "Name",
    firstName: "First Name",
    lastName: "Last Name",
    email: "Email",
    phone: "Phone",
    subject: "Subject",
    message: "Message",
    optional: "optional",
    date: "Date",
    location: "Location",
    price: "Price",
    download: "Download",
    upcoming: "Upcoming",
    past: "Past",
    calendar: "Event calendar",
    noEventsThisDay: "No events on this day.",
    searchEvents: "Search events…",
    noResults: "No events match your search.",
    ongoing: "Ongoing",
    completed: "Completed",
    orderNow: "Order via WhatsApp",
    outOfStock: "Out of stock",
    thankYou: "Thank you!",
    backHome: "Back to Home",
    keyPoints: "Key Points",
    whatToExpect: "What to Expect",
    agenda: "Agenda",
    registration: "Registration",
    objectives: "Objectives",
    outcomes: "Key Outcomes",
    features: "Key Features",
    benefits: "Benefits",
    faqs: "Frequently Asked Questions",
    beneficiaries: "Beneficiaries",
    gallery: "Gallery",
    nothingHere: "Nothing to show here yet.",
  },
  search: {
    label: "Search",
    placeholder: "Search the site…",
    startTyping: "Start typing to search across the whole site.",
    noResults: "No results found.",
    minChars: "Keep typing…",
  },
  home: {
    makeDonation: "Make a Donation",
    getSupport: "Get Support",
    subscribe: "Subscribe",
    emailPlaceholder: "Email address",
    subscribed: "Thank you for subscribing!",
    subscribeError: "Something went wrong — please try again.",
  },
  donate: {
    amount: "Amount (LKR)",
    donateNow: "Donate Now",
    successTitle: "Thank you for your donation!",
    successText: "Your payment was received. A confirmation has been recorded.",
    cancelledText: "Your donation was cancelled. You can try again at any time.",
    securePayment: "Secure online payment powered by PayHere",
    oneTime: "One-time",
    monthly: "Monthly",
    annual: "Annual",
    perMonth: "/month",
    chooseAmount: "Choose an amount",
    customAmount: "Custom amount",
    purpose: "Donation purpose",
    purposeGeneral: "Where it's needed most",
    purposeHealth: "Health services",
    purposeEducation: "Education & awareness",
    purposeCommunity: "Community programs",
    yourDetails: "Your details",
  },
  suggestions: {
    name: "Name (optional)",
    email: "Email (optional)",
    message: "Message",
  },
  business: {
    orderTitle: "Place your order",
    orderIntro: "Enter your details, then confirm the order with us on WhatsApp.",
    name: "Name",
    phone: "Phone number",
    email: "Email (optional)",
    quantity: "Quantity",
    fulfillment: "How would you like to receive it?",
    pickup: "Pickup",
    delivery: "Delivery",
    address: "Delivery address",
    notes: "Order notes (optional)",
    continueWhatsApp: "Save & continue to WhatsApp",
    submitting: "Saving order…",
    orderSaved: "Your order has been saved. Use this reference when contacting us:",
  },
  contact: {
    address: "Address",
    hours: "Office Hours",
    followUs: "Follow Us",
  },
  footer: {
    explore: "Explore",
    getInvolved: "Get Involved",
    rights: "All rights reserved.",
  },
};

const si: typeof en = {
  nav: {
    home: "මුල් පිටුව",
    about: "අප ගැන",
    projects: "ව්‍යාපෘති",
    services: "අපගේ සේවාවන්",
    servicesOverview: "සියලුම සේවාවන්",
    publications: "ප්‍රකාශන",
    news: "පුවත්",
    events: "සිදුවීම් සහ ගැලරිය",
    contact: "අප අමතන්න",
    donate: "පරිත්‍යාග",
    business: "ප්‍රජා ව්‍යාපාර",
    suggestions: "යෝජනා",
    media: "පුවත් සහ මාධ්‍ය",
    getInvolved: "සම්බන්ධ වන්න",
    menu: "මෙනුව",
    privacy: "පෞද්ගලිකත්ව ප්‍රතිපත්තිය",
    terms: "නියම සහ කොන්දේසි",
  },
  common: {
    readMore: "වැඩිදුර කියවන්න",
    learnMore: "තව දැනගන්න",
    viewAll: "සියල්ල බලන්න",
    submit: "යොමු කරන්න",
    send: "යවන්න",
    name: "නම",
    firstName: "මුල් නම",
    lastName: "අවසන් නම",
    email: "විද්‍යුත් තැපෑල",
    phone: "දුරකථන",
    subject: "මාතෘකාව",
    message: "පණිවිඩය",
    optional: "අත්‍යවශ්‍ය නොවේ",
    date: "දිනය",
    location: "ස්ථානය",
    price: "මිල",
    download: "බාගන්න",
    upcoming: "ඉදිරි",
    past: "පසුගිය",
    calendar: "සිදුවීම් දින දර්ශනය",
    noEventsThisDay: "මෙම දිනයේ සිදුවීම් නොමැත.",
    searchEvents: "සිදුවීම් සොයන්න…",
    noResults: "ඔබේ සෙවුමට ගැලපෙන සිදුවීම් නොමැත.",
    ongoing: "ක්‍රියාත්මක",
    completed: "සම්පූර්ණයි",
    orderNow: "WhatsApp හරහා ඇණවුම් කරන්න",
    outOfStock: "තොග අවසන්",
    thankYou: "ස්තූතියි!",
    backHome: "මුල් පිටුවට",
    keyPoints: "ප්‍රධාන කරුණු",
    whatToExpect: "ඔබට බලාපොරොත්තු විය හැකි දේ",
    agenda: "න්‍යාය පත්‍රය",
    registration: "ලියාපදිංචිය",
    objectives: "අරමුණු",
    outcomes: "ප්‍රධාන ප්‍රතිඵල",
    features: "ප්‍රධාන ලක්ෂණ",
    benefits: "ප්‍රතිලාභ",
    faqs: "නිතර අසන ප්‍රශ්න",
    beneficiaries: "ප්‍රතිලාභීන්",
    gallery: "ගැලරිය",
    nothingHere: "තවම පෙන්වීමට කිසිවක් නැත.",
  },
  search: {
    label: "සොයන්න",
    placeholder: "වෙබ් අඩවියේ සොයන්න…",
    startTyping: "සම්පූර්ණ වෙබ් අඩවියේ සෙවීමට ටයිප් කිරීම අරඹන්න.",
    noResults: "ප්‍රතිඵල හමු නොවීය.",
    minChars: "ටයිප් කරමින් සිටින්න…",
  },
  home: {
    makeDonation: "පරිත්‍යාගයක් කරන්න",
    getSupport: "සහාය ලබාගන්න",
    subscribe: "දායක වන්න",
    emailPlaceholder: "විද්‍යුත් තැපැල් ලිපිනය",
    subscribed: "දායක වීම සඳහා ස්තූතියි!",
    subscribeError: "යමක් වැරදුණි — කරුණාකර නැවත උත්සාහ කරන්න.",
  },
  donate: {
    amount: "මුදල (රු.)",
    donateNow: "දැන් පරිත්‍යාග කරන්න",
    successTitle: "ඔබගේ පරිත්‍යාගයට ස්තූතියි!",
    successText: "ඔබගේ ගෙවීම ලැබුණි. තහවුරු කිරීම සටහන් කර ඇත.",
    cancelledText: "ඔබගේ පරිත්‍යාගය අවලංගු විය. ඕනෑම වේලාවක නැවත උත්සාහ කළ හැක.",
    securePayment: "PayHere මගින් ආරක්ෂිත මාර්ගගත ගෙවීම",
    oneTime: "එක් වරක්",
    monthly: "මාසිකව",
    annual: "වාර්ෂිකව",
    perMonth: "/මසකට",
    chooseAmount: "මුදලක් තෝරන්න",
    customAmount: "වෙනත් මුදලක්",
    purpose: "පරිත්‍යාගයේ අරමුණ",
    purposeGeneral: "වඩාත්ම අවශ්‍ය තැනට",
    purposeHealth: "සෞඛ්‍ය සේවා",
    purposeEducation: "අධ්‍යාපනය සහ දැනුවත් කිරීම",
    purposeCommunity: "ප්‍රජා වැඩසටහන්",
    yourDetails: "ඔබගේ විස්තර",
  },
  suggestions: {
    name: "නම (අත්‍යවශ්‍ය නොවේ)",
    email: "විද්‍යුත් තැපෑල (අත්‍යවශ්‍ය නොවේ)",
    message: "පණිවිඩය",
  },
  business: {
    orderTitle: "ඔබගේ ඇණවුම ලබා දෙන්න",
    orderIntro: "ඔබගේ විස්තර ඇතුළත් කර WhatsApp හරහා ඇණවුම තහවුරු කරන්න.",
    name: "නම",
    phone: "දුරකථන අංකය",
    email: "විද්‍යුත් තැපෑල (අත්‍යවශ්‍ය නොවේ)",
    quantity: "ප්‍රමාණය",
    fulfillment: "ඔබට භාණ්ඩය ලබා ගැනීමට අවශ්‍ය ආකාරය",
    pickup: "පැමිණ ලබා ගැනීම",
    delivery: "බෙදා හැරීම",
    address: "බෙදා හැරීමේ ලිපිනය",
    notes: "ඇණවුම් සටහන් (අත්‍යවශ්‍ය නොවේ)",
    continueWhatsApp: "සුරකින්න සහ WhatsApp වෙත යන්න",
    submitting: "ඇණවුම සුරකිමින්…",
    orderSaved: "ඔබගේ ඇණවුම සුරකින ලදී. අප අමතන විට මෙම යොමු අංකය භාවිතා කරන්න:",
  },
  contact: {
    address: "ලිපිනය",
    hours: "කාර්යාල වේලාවන්",
    followUs: "අපව අනුගමනය කරන්න",
  },
  footer: {
    explore: "ගවේෂණය",
    getInvolved: "සම්බන්ධ වන්න",
    rights: "සියලුම හිමිකම් ඇවිරිණි.",
  },
};

const ta: typeof en = {
  nav: {
    home: "முகப்பு",
    about: "எங்களைப் பற்றி",
    projects: "திட்டங்கள்",
    services: "எங்கள் சேவைகள்",
    servicesOverview: "அனைத்து சேவைகளும்",
    publications: "வெளியீடுகள்",
    news: "செய்திகள்",
    events: "நிகழ்வுகள் & படத்தொகுப்பு",
    contact: "தொடர்பு கொள்ள",
    donate: "நன்கொடை",
    business: "சமூக வணிகம்",
    suggestions: "ஆலோசனைகள்",
    media: "செய்திகள் & ஊடகம்",
    getInvolved: "பங்கேற்க",
    menu: "பட்டியல்",
    privacy: "தனியுரிமைக் கொள்கை",
    terms: "விதிமுறைகள் மற்றும் நிபந்தனைகள்",
  },
  common: {
    readMore: "மேலும் வாசிக்க",
    learnMore: "மேலும் அறிய",
    viewAll: "அனைத்தையும் காண்க",
    submit: "சமர்ப்பிக்க",
    send: "அனுப்பு",
    name: "பெயர்",
    firstName: "முதல் பெயர்",
    lastName: "கடைசி பெயர்",
    email: "மின்னஞ்சல்",
    phone: "தொலைபேசி",
    subject: "தலைப்பு",
    message: "செய்தி",
    optional: "விருப்பத்திற்குரியது",
    date: "திகதி",
    location: "இடம்",
    price: "விலை",
    download: "பதிவிறக்க",
    upcoming: "வரவிருக்கும்",
    past: "கடந்த",
    calendar: "நிகழ்வு நாட்காட்டி",
    noEventsThisDay: "இந்த நாளில் நிகழ்வுகள் இல்லை.",
    searchEvents: "நிகழ்வுகளைத் தேடுங்கள்…",
    noResults: "உங்கள் தேடலுக்குப் பொருந்தும் நிகழ்வுகள் இல்லை.",
    ongoing: "நடைபெறும்",
    completed: "நிறைவு",
    orderNow: "WhatsApp மூலம் ஆர்டர் செய்ய",
    outOfStock: "கையிருப்பு இல்லை",
    thankYou: "நன்றி!",
    backHome: "முகப்புக்கு திரும்ப",
    keyPoints: "முக்கிய அம்சங்கள்",
    whatToExpect: "எதிர்பார்க்கக்கூடியவை",
    agenda: "நிகழ்ச்சி நிரல்",
    registration: "பதிவு",
    objectives: "நோக்கங்கள்",
    outcomes: "முக்கிய விளைவுகள்",
    features: "முக்கிய அம்சங்கள்",
    benefits: "நன்மைகள்",
    faqs: "அடிக்கடி கேட்கப்படும் கேள்விகள்",
    beneficiaries: "பயனாளிகள்",
    gallery: "படத்தொகுப்பு",
    nothingHere: "இன்னும் காட்ட எதுவும் இல்லை.",
  },
  search: {
    label: "தேடல்",
    placeholder: "தளத்தில் தேடுங்கள்…",
    startTyping: "முழு தளத்திலும் தேட தட்டச்சு செய்யத் தொடங்குங்கள்.",
    noResults: "முடிவுகள் எதுவும் இல்லை.",
    minChars: "தட்டச்சு செய்யுங்கள்…",
  },
  home: {
    makeDonation: "நன்கொடை வழங்க",
    getSupport: "உதவி பெற",
    subscribe: "பதிவு செய்ய",
    emailPlaceholder: "மின்னஞ்சல் முகவரி",
    subscribed: "பதிவு செய்ததற்கு நன்றி!",
    subscribeError: "ஏதோ தவறு நடந்தது — மீண்டும் முயற்சிக்கவும்.",
  },
  donate: {
    amount: "தொகை (ரூ.)",
    donateNow: "இப்போது நன்கொடை வழங்க",
    successTitle: "உங்கள் நன்கொடைக்கு நன்றி!",
    successText: "உங்கள் கட்டணம் பெறப்பட்டது. உறுதிப்படுத்தல் பதிவு செய்யப்பட்டுள்ளது.",
    cancelledText: "உங்கள் நன்கொடை ரத்து செய்யப்பட்டது. எப்போது வேண்டுமானாலும் மீண்டும் முயற்சிக்கலாம்.",
    securePayment: "PayHere மூலம் பாதுகாப்பான ஆன்லைன் கட்டணம்",
    oneTime: "ஒருமுறை",
    monthly: "மாதாந்தம்",
    annual: "வருடாந்தம்",
    perMonth: "/மாதம்",
    chooseAmount: "தொகையைத் தேர்ந்தெடுக்கவும்",
    customAmount: "வேறு தொகை",
    purpose: "நன்கொடையின் நோக்கம்",
    purposeGeneral: "மிகவும் தேவையான இடத்திற்கு",
    purposeHealth: "சுகாதார சேவைகள்",
    purposeEducation: "கல்வி & விழிப்புணர்வு",
    purposeCommunity: "சமூக திட்டங்கள்",
    yourDetails: "உங்கள் விவரங்கள்",
  },
  suggestions: {
    name: "பெயர் (விருப்பத்தேர்வு)",
    email: "மின்னஞ்சல் (விருப்பத்தேர்வு)",
    message: "தகவல்",
  },
  business: {
    orderTitle: "உங்கள் ஆர்டரை பதிவு செய்யுங்கள்",
    orderIntro: "உங்கள் விவரங்களை உள்ளிட்டு, WhatsApp மூலம் ஆர்டரை உறுதிப்படுத்துங்கள்.",
    name: "பெயர்",
    phone: "தொலைபேசி எண்",
    email: "மின்னஞ்சல் (விருப்பத்தேர்வு)",
    quantity: "அளவு",
    fulfillment: "பொருளை எவ்வாறு பெற விரும்புகிறீர்கள்?",
    pickup: "நேரில் பெறுதல்",
    delivery: "விநியோகம்",
    address: "விநியோக முகவரி",
    notes: "ஆர்டர் குறிப்புகள் (விருப்பத்தேர்வு)",
    continueWhatsApp: "சேமித்து WhatsApp-க்கு செல்லவும்",
    submitting: "ஆர்டர் சேமிக்கப்படுகிறது…",
    orderSaved: "உங்கள் ஆர்டர் சேமிக்கப்பட்டது. எங்களைத் தொடர்புகொள்ளும்போது இந்த எண்ணைப் பயன்படுத்தவும்:",
  },
  contact: {
    address: "முகவரி",
    hours: "அலுவலக நேரம்",
    followUs: "எங்களைப் பின்தொடரவும்",
  },
  footer: {
    explore: "மேலும் பார்க்க",
    getInvolved: "பங்கேற்க",
    rights: "அனைத்து உரிமைகளும் பாதுகாக்கப்பட்டவை.",
  },
};

export const dictionaries = { en, si, ta };

export type Dictionary = typeof en;
export type DictionaryGroup = keyof Dictionary;

/** Built-in labels for a locale, ignoring any admin overrides. */
export function getDictionary(locale: Locale): Dictionary {
  return dictionaries[locale] ?? en;
}
